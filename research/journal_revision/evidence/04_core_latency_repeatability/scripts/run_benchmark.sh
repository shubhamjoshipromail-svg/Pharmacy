...
TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/rxcheck-benchmark.XXXXXX")
...
DB_USER=rxcheck_benchmark
DB_NAME=rxcheck_benchmark

mkdir -p "$RAW_DIR" "$LOG_DIR"
rm -f "$RAW_DIR/benchmark_results.json" "$RAW_DIR/measurements.csv" "$RAW_DIR/environment_lock.txt"
rm -f "$LOG_DIR/postgres.log" "$LOG_DIR/benchmark.log"
...
DATABASE_URL="postgresql+psycopg2://${DB_USER}@127.0.0.1:${PORT}/${DB_NAME}" \
  "$PYTHON_BIN" "$SCRIPT_DIR/run_core_benchmark.py" \
    --output-json "$RAW_DIR/benchmark_results.json" \
    --output-csv "$RAW_DIR/measurements.csv" \
