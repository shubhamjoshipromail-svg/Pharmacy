...
class InteractionCheckRun(Base):
    ...
    run_by: Mapped[str] = mapped_column(ForeignKey("users.id"), nullable=False)
    run_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    medications_snapshot: Mapped[Any] = mapped_column(JSON, nullable=False)
    sources_used: Mapped[list[str]] = mapped_column(JSON, nullable=False)
...
class InteractionCheckFinding(Base):
    ...
    run_id: Mapped[str] = mapped_column(ForeignKey("interaction_check_runs.id", ondelete="CASCADE"), nullable=False)
    ...
    sources_at_run: Mapped[list[str]] = mapped_column(JSON, nullable=False)
    sources_conflicted: Mapped[bool] = mapped_column(Boolean, nullable=False)
    llm_explanation_id: Mapped[Optional[str]] = mapped_column(ForeignKey("llm_explanations.id"), nullable=True)
...
class LlmExplanation(Base):
    ...
    structured_input: Mapped[Any] = mapped_column(JSON, nullable=False)
    ...
    validation_errors: Mapped[Any] = mapped_column(JSON, nullable=True)
    generated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    latency_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    token_usage: Mapped[Any] = mapped_column(JSON, nullable=True)
